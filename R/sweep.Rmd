---
title: "E&A World Cup 2026 Sweep"
output: 
  html_document:
    toc: TRUE
    toc_float: TRUE
...
now <- Sys.time()
# now <- as.POSIXct("2026-06-12 12:00:00")
groups <- c("Group A",
            "Group B",
            "Group C",
            "Group D",
...
Welcome to the World Cup 2026 Sweep! Your chance to add a little spice to your
...
rankings <- read_csv(here("data", "rankings.csv"))


rankings <- rankings |> 
  mutate(team1 = country, 
         fifa_rank1 = fifa_rank,
         sweep_rank1 = sweep_rank,
         team2 = lead(country, 24),
         fifa_rank2 = lead(fifa_rank, 24),
         sweep_rank2 = lead(sweep_rank, 24),
         draw = glue("{team1} ({sweep_rank1}) {team2} ({sweep_rank2})")) |> 
  filter(!is.na(team2))

contestants |> 
  left_join(rankings,by = join_by(Team == sweep_rank)) |> 
  select(Number, Name, Teams = draw, Paid) |> 
  reactable(pagination = FALSE, 
            filterable = TRUE, 
            compact = TRUE, 
            columns = list(
              Number = colDef(name = "#", minWidth = 10),
              Name = colDef(name = "Name", minWidth = 50),
              Teams = colDef(name = "Teams", minWidth = 50),
              Paid = colDef(name = "Paid?", minWidth = 10))
            )
...
As you would expect, there are data-driven categories with a value of £6 each:
...
And finally because data isn't everything there is a qualitative category with a £6 value:

- **The Mystery Prize** To Be Decided

#### The small print

All statistics will be drawn from the official
[World Cup 2026](https://www.fifa.com/en/tournaments/mens/worldcup/canadamexicousa2026) website. The organisers reserve the right to tweak the criteria for the bonus categories, to break ties or
...
fixtures |> 
  ...
  reactable(pagination = FALSE,
            ...
            filterable = TRUE,
            columns = list(
            matchnumber = colDef(name = "Match #", minWidth = 55),
            date = colDef(name = "Date", format = colFormat(datetime = TRUE), minWidth = 150),
            fixture = colDef(name = "Fixture", minWidth = 200),
            location = colDef(name = "Venue"),
            group = colDef(name= "Group", minWidth = 60),
            roundnumber = colDef(name = "Round", minWidth = 50))
            )
...
The 'anti-football index' is calculated as follows: a red card is 25pts, a yellow 5pts
...
matchdata_full |> 
  ...
  reactable(pagination = TRUE,
            ...
            columns = list(
              ...
              rcards = colDef(name = "Red cards"),
              fouls = colDef(name = "Fouls"),
              af_index = colDef(name = "AF index", format = colFormat(digits = 2))
            ))
